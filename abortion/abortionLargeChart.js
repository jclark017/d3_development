function generateAbortionLargeChart () {
  ...
  d3.json('presidents.json', function (errorPres, presData) {
    ...
    var tip = d3
      ...
      .offset([-10, 0])
      .html(function (d) {
        if (d.occurencesPer1000Live > 0) {
          occurencesPer1000LiveDescription = 'Deficit'
        } else {
          occurencesPer1000LiveDescription = 'Surplus'
        }

        if (d.changeFromPreceding > 0) {
          if (occurencesPer1000LiveDescription == 'Surplus') {
        ...
        ret += '<br/><strong>' + d.Year + ' ' + occurencesPer1000LiveDescription + ":</strong> <span style='color:white'>$" +
          Math.abs(d.occurencesPer1000Live) + 'bn</span>'
        ret += '<br/><strong>' + occurencesPer1000LiveDescription + " Change:</strong> <span style='color:white'>$" + Math.abs(d.changeFromPreceding) +
    ...
    d3.csv('abortion.csv', type, function (error, dataProto) {
      //Convert data formats
      var data = dataProto.map(function (currentObject) {
        return {
          Year: currentObject.Year,
          occurences: parseFloat(currentObject.occurences),
          occurencesPer1000Live: parseFloat(currentObject.occurencesPer1000Live),
          changeFromPreceding: parseFloat(currentObject.changeFromPreceding),
      ...
      yScaleRight
        .domain([
          0,
          d3.max(data, function (d) {
            return d.occurencesPer1000Live
          })
        ])
        .range([0, chartHeight])
      //Scale with 0 lower bound
      ...
      var lineFunction = d3.svg
        ...
        .y(function (d) {
          return yAxisScaleRight(d.occurencesPer1000Live)
        })
      ...
      svg
        ...
        .duration(500)
        .attr('y', function (d) {
          return chartHeight - Math.max(0, yScale(d.changeFromPreceding))
        })
        .attr('height', function (d) {
          return Math.abs(yScale(d.changeFromPreceding))
        })
        .delay(function (d, i) {
          if (i == 0) {
            return 1000
